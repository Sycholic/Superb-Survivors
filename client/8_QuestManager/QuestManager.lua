 ...
 --todo
	...
	for i=0,#Actions do
		if(Actions[i]) then
			...
			elseif(Actions[i].ActionType == "setNPCWantsToTalk") then -- adds the "!" exclamation mark
				local TargetSurvivor = SSM:GetSurvivorByName(Actions[i].NPCName)
			...
			and (trigger ~= false)  then
				...
				then
					...
					if (trigger.Enabled) then
						...
						print("SSQM: quest/trigger " .. tostring(trigger.TriggerName) .. " disabled therefore not Triggered w " .. tostring(Param1))
					end
...
--and add the global quest manager object
...
function QuestManagerKeyDownHandle(keyNum)
	--[[	Cleaned up the several calls to getSpecificPlayer to one.
            Added in the nil checks because some reason all bound keys can open their windows
            prior to a player actually being loaded in world before hitting 'click to start'
            QuestManager and SuperSurvivor windows effected and fixed so far
            Suspect the other errors that pop soon as clicking C2S are same issue but cant find errors to track it
	local player = getSpecificPlayer(0)
	--]]
...
--[[ excluding this for now but keeping it just in case. others binds actually did nothing at all and removed them
	elseif( keyNum == 199) then -- home key
		if (myQuestInfoWindow ~= nil) then myQuestInfoWindow:setVisible(not myQuestInfoWindow:getIsVisible()) end
